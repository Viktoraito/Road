...
float l_obst,f_obst,r_obst,b_obst;
int procIsAsking;
int x,y;
float **map;
...
for(int i=0; i<3; i++) {
  ...
  dx--; dy--;
  if(dx>0)
    l_obst=(map[dx][dy-1]<1.0?1.0:0.0);
  else
    l_obst=1.0;
  if(dy>0)
    f_obst=(map[dx-1][dy]<1.0?1.0:0.0);
  else
    f_obst=1.0;
  if(dx<x-1)
    r_obst=(map[dx][dy+1]<1.0?1.0:0.0);
  else
    r_obst=1.0;
  if(dy<y-1)
    b_obst=(map[dx+1][dy]<1.0?1.0:0.0);
  else
    b_obst=1.0;

  //printf("obst %d %f %f %f %f\n",c_num,l_obst,f_obst,r_obst,b_obst);
  ...
  if (rc < 0) {
    ...
    if (rc < 0) {
      ...
      rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "BACK", "%f", b_obst);
      if (rc < 0) {
        printf("Error at xmlTextWriterWriteAttribute\n");
        return;
      }
...
int main(int argc, char **argv)
{
  ...
  if(c_num<1 || c_num>3) {
   ...
   char tmp[10];
   sprintf(tmp,"%d",c_num);
   strcpy(reader,"readerO"); 
   strcat(reader,tmp);
   strcat(reader,".xml");
   strcpy(data,"obstData");
   strcat(data,tmp);
   strcat(data,".xml");

  serv_sock = socket(AF_INET, SOCK_STREAM, 0);
  if(!conn_serv(serv_sock,port)) {
    printf("CLient_obst: Can't connect with server\n");
    exit(EXIT_FAILURE);
  }
